<div>
    <div class="bg-gray-900 border-b border-gray-800 safe-top p-4 sticky top-0 z-10 flex items-center gap-3">
        ...
        @endif
        <h1 class="text-xl font-bold text-white">Transferência</h1>
    </div>

    <div class="p-4 page-content">
        @if($modo === 'scan_lote')
            <div class="text-center py-12">
                <div class="w-24 h-24 bg-purple-500/20 text-purple-500 rounded-full flex items-center justify-center mx-auto mb-6">
                ...
                <h2 class="text-2xl font-bold text-white mb-2">Escanear Lote</h2>
                <p class="text-gray-400 mb-8">Escaneie o código de barras do lote que deseja transferir.</p>
        ...
        @elseif($modo === 'scan_destino')
            <div class="bg-gray-800 rounded-xl p-4 border border-gray-700 mb-6">
                <div class="text-gray-400 text-sm mb-1">Lote Origem</div>
                ...
                <div class="flex justify-between items-center mt-2">
                    ...
                    <span class="bg-purple-500/20 text-purple-400 text-xs px-2 py-1 rounded font-bold">Qtd: {{ $loteSelecionado->quantidade_atual }}</span>
                </div>
                <div class="mt-4 pt-4 border-t border-gray-700">
                    <div class="text-gray-400 text-sm">Endereço Atual</div>
                    <div class="text-lg font-bold text-white">{{ $loteSelecionado->endereco->corredor }}-{{ $loteSelecionado->endereco->estante }}-{{ $loteSelecionado->endereco->nivel }}</div>
                </div>
            </div>

            <div class="text-center py-6">
                @if(!$enderecoDestino)
                    <h2 class="text-xl font-bold text-white mb-2">Escanear Destino</h2>
                    <p class="text-gray-400 mb-6">Escaneie o código de barras da prateleira de destino.</p>
                    <div class="p-4 bg-gray-800 border border-gray-700 rounded-xl flex items-center gap-4">
                        <svg class="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                        <div class="text-left">
                            <div class="text-white font-bold">Leitor Ativo</div>
                ...
                @else
                    <div class="bg-purple-500/10 rounded-xl p-4 border border-purple-500/20 mb-6">
                        <div class="text-purple-400 text-sm mb-1">Novo Endereço Destino</div>
                        <div class="text-2xl font-bold text-white">{{ $enderecoDestino->corredor }}-{{ $enderecoDestino->estante }}-{{ $enderecoDestino->nivel }}</div>
                    </div>
                    
                    <button wire:click="confirmarTransferencia" class="w-full py-4 rounded-xl font-bold text-lg bg-brand-500 text-gray-900 shadow-[0_0_15px_rgba(245,158,11,0.5)]">
        ...
        @elseif($modo === 'concluido')
            <div class="text-center py-12">
                ...
                <h2 class="text-2xl font-bold text-white mb-2">Transferência Concluída!</h2>
                <p class="text-gray-400 mb-8">Lote transferido para o novo endereço com sucesso.</p>
                <button wire:click="voltar" class="bg-gray-800 text-white w-full py-4 rounded-xl font-bold">Nova Transferência</button>
            </div>
        @endif
    </div>
</div>
